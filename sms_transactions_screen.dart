// lib/screens/sms_transactions_screen.dart
...
class _SmsTransactionsScreenState extends State<SmsTransactionsScreen> {
  ...
  Future<void> _accept(ParsedSms sms) async {
    ...
    // Use current user as the member
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('✓ ${state.formatAmount(sms.amount)} saved'),
  ...
  void _discard(ParsedSms sms) {
    setState(() {
      _discarded.add(sms.smsId);
      _pending.removeWhere((s) => s.smsId == sms.smsId);
    });
  }
  ...
  Future<void> _editAndAccept(ParsedSms sms) async {
      ...
      builder: (_) => StatefulBuilder(
        builder: (ctx, setSheet) => Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(sms.rawBody.length > 80 ? '${sms.rawBody.substring(0, 80)}...' : sms.rawBody,
            ...
            Container(
              ...
              child: Row(children: [
                Icon(sms.isIncome ? Icons.arrow_downward_rounded : Icons.arrow_upward_rounded,
    ...
    if (confirmed == true) {
      // Accept with potentially edited category
      final edited = ParsedSms(
        amount: sms.amount, isIncome: sms.isIncome,
        category: selectedCategory, source: sms.source,
        rawBody: sms.rawBody, date: sms.date, smsId: sms.smsId,
      );
  ...
  Widget _buildPermissionPrompt() => Center(
    child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 20),
        const Text('SMS Permission Required', style: TextStyle(
  ...
  Widget _buildEmpty() => Center(
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 16),
      const Text('No new transactions found', style: TextStyle(
        fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
      const SizedBox(height: 8),
      const Text('Scanned last 7 days of SMS from banks\nand JazzCash / EasyPaisa',
        textAlign: TextAlign.center,
