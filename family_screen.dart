...
class FamilyScreen extends StatelessWidget {
        ...
        actions: [
          if (isAdmin)
            IconButton(
              ...
              tooltip: 'Add Member',
              onPressed: () => _showMemberForm(context, state, null),
            ),
        ],
      ),
      body: ListView(
        ...
        children: [
                  ...
                  GestureDetector(
                    onTap: () {
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content: Text('Code: \${state.household!.inviteCode} — share with family'),
          ...
          ...members.map((m) => _MemberCard(
            ...
            onEdit:   () => _showMemberForm(context, state, m),
  ...
  void _showMemberForm(BuildContext context, AppState state, AppMember? existing) {
    ...
    MemberRole selectedRole = existing?.role ?? MemberRole.spender;
              ...
              child: ElevatedButton(
                onPressed: () {
                  ...
                  if (isEdit) {
                    ...
                    state.addMember(AppMember(
                      id:          'm${DateTime.now().millisecondsSinceEpoch}',
                      name:        name,
                      phone:       phone,
                      role:        selectedRole,
                      householdId: state.household?.id ?? '',
                      color: [AppColors.member1, AppColors.member2, AppColors.member3,
                              const Color(0xFF4A90D9)][state.members.length % 4],
                    ));
                  }
